...
@implementation TitaniumCalendarObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_titaniumcalendar);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any TitaniumCalendar patches in JS before a context is loaded... nice for 
...
{
	// create TitaniumCalendar global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_titaniumcalendar = [[TitaniumCalendarObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_titaniumcalendar];
	
	NSString *_titaniumcalendarNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _titaniumcalendarNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_titaniumcalendar valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.titaniumcalendar.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
